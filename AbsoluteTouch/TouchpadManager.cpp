...
HRESULT STDMETHODCALLTYPE TouchpadManager::OnSynDevicePacket(long seqNum)
{
  ...
  long buttonState;
  m_packet->GetProperty(SP_ButtonState, &buttonState);
  bool touching = (fingerState & SF_FingerPresent) != 0;
  bool clicking = (buttonState & SF_ButtonReportedLeft) != 0;
  bool startToClick = false;
  if (clicking) {
    if (!m_clicking) {
      m_clicking = true;
      startToClick = true;
  ...
  if (touching) {
    ...
    if (m_touching) {
      m_callback->OnTouchMoved(point, startToClick);
    } else {
      m_callback->OnTouchStarted(point, startToClick);
      m_touching = true;
    }
  } else if (m_touching) {
